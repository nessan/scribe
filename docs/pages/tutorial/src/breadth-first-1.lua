-----------------------------------------------------------------------------------------------------------------------
-- Scribe Tutorial: Demonstrate breadth first table traversal in the `metadata` method.
--
...
local function metadata(tbl, md)
    ...
    for _, v in pairs(tbl) do
        ...
        if type(v) == 'table' then
            subs = subs + 1
            if not md[v] then table.insert(children, v) end
        end
    end
    md[tbl].size, md[tbl].array, md[tbl].subs = size, array, subs
    for _, child in ipairs(children) do metadata(child, md) end
    return md
end
