-----------------------------------------------------------------------------------------------------------------------
-- Scribe Tutorial: Using `scribe` to implement a custom `__tostring` metamethod.
--
...
function scribe(obj, opts, overrides)
    if type(obj) ~= 'table' then return simple_string(obj) end

    if options == nil then return table_string(obj, options.pretty) end
...
function pretty(tbl, overrides)
    return scribe(tbl, options.pretty, overrides)
end

function inline(tbl, overrides)
    return scribe(tbl, options.inline, overrides)
end

function classic(tbl, overrides)
    return scribe(tbl, options.classic, overrides)
end

function alt(tbl, overrides)
    return scribe(tbl, options.alt, overrides)
end

function json(tbl, overrides)
    return scribe(tbl, options.json, overrides)
end

function inline_json(tbl, overrides)
    return scribe(tbl, options.inline_json, overrides)
end
...
function mt.__tostring(tbl)
    count = count + 1
    local tbl_options = { use_metatable = false }
