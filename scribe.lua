...
--- Private function: Returns an ordered iterator that traverses a table in a sorted key order.
...
local function table_string(root_tbl, opts)
    ...
    local function process(tbl, path)
        ...
        -- The current element index is `i`.
        local i = 0

        -- Grab an appropriate iterator used to traverse the top-level elements in `tbl`
        ...
        for k, v in iter(tbl) do
            ...
            if type(v) == 'table' then
                if md[v].path then
                    ...
                    -- If `v` is a shared table we bettter show the associated key no matter what
